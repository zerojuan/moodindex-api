...
describe('React', () => {
  ...
  it('- Should NotFound on non-existent mood', async () => {
    ...
    await expect(React(fixture)).rejects.toMatchObject(new errors.NotFoundError(
  ...
  it('- Should BadRequest when value is too large', async () => {
    ...
    await expect(React(fixture)).rejects.toMatchObject(new errors.BadRequestError(
      'BadRequestError: value out of range'
    ))
  })
